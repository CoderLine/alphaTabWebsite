...
* If chunk splitting is active, it puts alphaTab into a custom chunk to ensure the code can be shared in an optimal way.
...
<Tabs defaultValue="source-dir" values={[
    ...
    { label: "Chunk configuration", value: "chunk"},
...
If the WebPack configuration already has chunk optimization enabled through [`optimization.splitChunks.cacheGroups`](https://webpack.js.org/plugins/split-chunks-plugin/)
alphaTab will extend this configuration to create a dedicated chunk for alphaTab.
This customization can either be disabled or adjusted: 

```js
// disable custom chunk 
new AlphaTabWebPackPlugin({
    alphaTabChunk: false
})
...
WebPack comes with built-in support for Web Workers and alphaTab, but [not for Audio Worklets](https://github.com/webpack/webpack/issues/11543). 
...
:::warning 
If this audio worklet support is disabled this might break the audio playback through worklets. You might configure alphaTab to use the 
[`player.outputMode`](/docs/reference/settings/player/outputmode) of alphaTab to use always the ScriptProcessorNode.
:::
