...
object. The main namespace `alphaTab` contains every class and enum exposed by the API. The main api is the `alphaTab.platform.javaScript.AlphaTabApi`
class:
...
Please refer to the [API Reference](http://localhost:3000/docs/reference/events) to find a full list of events available.
...
```js
element.addEventListener('alphaTab.loaded', (e) => {
...
```js
$(element).on('alphaTab.loaded', (e, score) => {
    console.log('Score was loaded!', score);
});
```
