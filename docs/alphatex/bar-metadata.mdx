...
`\ae (BarNumber BarNumber...)` or just `\ae BarNumber` if only one number.
The BarNumber specifies in which repetition the ending bar should be played.
...
To split your song into individual sections (e.g. intro, chorus, solo etc.), you can mark a bar as a section start by using `\section` followed by the section name.
