...
A bar is completed by a pipe `|` symbol or a new metadata tag. 
...
\\ks A
    // bar 2 contents
    C4 
// bar 3 metadata tags (notice the missing pipe, bar 3 start is indicated by the time signature tag)
\\ts (3 4)
    C4
`}</AlphaTexSample>

Metadata tags need to be at the start of bar followed by the content as a new metadata tag is also a marker for a new bar.
This can be confusing for tags which control display of elements at the end of the bar. e.g. a repeat end or a `barLineRight` are 
still declared in the start of a bar. 
...
Specifically `\barLineRight dotted` is important in this example, it starts bar 3 and specifies the right barline of it.
This behavior exists for backwards compatibility. In future alphaTex versions we will make the `|` mandatory setting clear boundaries 
of what belongs to a bar.
...
More details are described over at the [`\\articulation`](./staff-metadata.mdx#articulation) metadata docs.

<AlphaTexSample>{`
\\track { instrument percussion }
...
* The accidental (`#`, `b`, `bb`, `x`, `##`,..; see [Accidentals](./note-properties.mdx#acc) for details on accidental handling)
* The octave (a number)
