...
the rough DOM structure according to the final control. Also we want our control to fit 80% of
...
In this tutorial we will use the demo file hosted by alphaTab at `https://www.alphatab.net/files/canon.gp`
...
But until now the overlay is always visible. So we listen to two of the main events of alphaTab
...
Now we got our alphaTab control, displaying an input file and showing
an indicator to the end-user while it is loading. alphaTab 
takes care of all needed bits to load the file via Ajax, render
the music sheet and listen on certain events like resizes to scale
the music sheet. There is a huge list of settings where you can adjust
the display of alphaTab which can be found in the [reference docs](/docs/reference/settings)

This will allow to adjust which elements are shown, how the music sheet is layed out, 
which tracks to display and other internals. 
