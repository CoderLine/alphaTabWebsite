...
The track selector will go into the sidebar, so let's extend our `index.html` with a template that we will use for all items in the track selector and
lets add some place in the sidebar where the items will go.
...
In the details we show the name and other future controls for volume and muting could go there. 
In this tutorial we will use FontAwesome to have some icons.
...
Now the real magic happens as we will hook this up with alphaTab. We will listen again to some events and fill the UI accordingly. 
This will be also the first time that we signal based on some UI interaction that alphaTab should do something. 
...
Last but not least we add another listener to the [`renderStarted`](/docs/reference/api/renderstarted) event which will mark the currently rendered
